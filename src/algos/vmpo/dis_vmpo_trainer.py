# -*- encoding: utf-8 -*-
'''
@File       :dis_vmpo_trainer.py
@Description:Discrete V-MPO (Virtual Continuous Multi-Step Off-Policy Correction) Algorithm Trainer
@Date       :2025/04/24
@Author     :Trae
@Version    :python
'''
...
class DiscreteVMPOTeacherTrainer(BaseTrainer):
    ...
    def update(self, data):
        """
        Perform one update step for Discrete V-MPO Teacher
        """
        b_obs, b_actions, b_returns, b_values, b_logprobs = data
        
        # Get new values and log probabilities
        _, new_logprob, entropy, new_value, _ = self.agent.get_action_and_value(b_obs, b_actions.long())
