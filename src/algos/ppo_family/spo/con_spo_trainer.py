...
class Continous_SPO_Trainer(BaseTrainer):
    ...
    def compute_policy_loss(self, mb_advantages, ratio1):
        ...
        Returns:
            _type_: _description_
        """
        pg_loss_1 = - mb_advantages * ratio1
        pg_loss_2 = (0.5 * torch.abs(mb_advantages) / self.clip_coef) * (ratio1 - 1)**2
        
        pg_loss = (pg_loss_1 + pg_loss_2).mean()
        
        return pg_loss, pg_loss_1.mean().item(), pg_loss_2.mean().item()
